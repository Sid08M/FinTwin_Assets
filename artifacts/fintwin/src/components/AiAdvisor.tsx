...
export function AiAdvisor({ data }: AiAdvisorProps) {
  ...
  const { messages, sendMessage, mode, setMode, isGenerating } = useAiChat(data);
            ...
            <div className={`p-4 border-b border-white/10 flex items-center justify-between transition-colors ${mode === 'roast' ? 'bg-destructive/10' : 'bg-slate-900/50'}`}>
              <div className="flex items-center gap-3">
                <div className="relative">
                  <img 
                    src={`${import.meta.env.BASE_URL}images/avatar-${mode}.png`} 
                    alt="AI Avatar" 
                    className="w-10 h-10 rounded-full object-cover border border-white/20 bg-black"
                  />
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{mode === 'professional' ? 'Strategic Analysis' : 'Brutal Honesty'}</p>
                </div>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 p-4 overflow-y-auto custom-scrollbar space-y-4">
              {messages.length === 0 && (
                <div className="h-full flex flex-col items-center justify-center text-center px-4 opacity-50">
                  ...
                  <p className="text-xs text-slate-500 mt-1">"How can I reach $1M faster?"</p>
                </div>
              )}
