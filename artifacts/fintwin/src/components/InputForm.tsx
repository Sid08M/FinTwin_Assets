  ...
  const InputRow = ({ 
    ...
    <div className="group space-y-2">
      ...
      <div className="relative">
        {prefix && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-medium">{prefix}</span>}
        <input
          type="number"
          value={value || ""}
        ...
        />
        {suffix && <span className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 font-medium">{suffix}</span>}
      </div>
    </div>
  );
    ...
    <div className="glass-panel rounded-2xl p-6">
      ...
      <div className="space-y-6">
        <div className="space-y-6 pb-6 border-b border-white/5">
          ...
          <InputRow label="Monthly Savings" value={data.monthlySavings} field="monthlySavings" icon={Wallet} prefix="$" />
        </div>

        <div className="space-y-6 pb-6 border-b border-white/5">
          <h3 className="text-xs uppercase tracking-wider text-cyan-500 font-bold">Expenses</h3>
          <InputRow label="Needs (Rent, Groceries)" value={data.monthlyExpensesNeeds} field="monthlyExpensesNeeds" icon={Activity} prefix="$" />
          <InputRow label="Wants (Dining, Hobbies)" value={data.monthlyExpensesWants} field="monthlyExpensesWants" icon={Target} prefix="$" />
        </div>

        <div className="space-y-6 pb-6 border-b border-white/5">
          <h3 className="text-xs uppercase tracking-wider text-purple-400 font-bold">Future Horizon</h3>
          <InputRow label="Current Saved Capital" value={data.currentSavings} field="currentSavings" icon={DollarSign} prefix="$" step={1000} />
          <InputRow label="Expected Annual Return" value={data.annualReturn} field="annualReturn" icon={Percent} suffix="%" step={0.5} />
          <InputRow label="Simulation Years" value={data.years} field="years" icon={Calendar} suffix="yrs" step={1} />
        </div>
      </div>
    </div>
  );
